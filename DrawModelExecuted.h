	...
	matrix* bone_to_world, float* flex_weights, float* flex_delayed_weights, const Vector& model_origin, int flags)
{
	...
	if (info.pStudioHdr != nullptr && csgo->local != nullptr) {
		if (interfaces.engine->IsConnected() && interfaces.engine->IsInGame()) {
			...
			static IMaterial* mat5 = interfaces.material_system->FindMaterial("particle/fire_burning_character/fire_env_fire", "Other textures");
			static IMaterial* mat6 = interfaces.material_system->FindMaterial("particle/fire_burning_character/fire_env_fire_depthblend", "Other textures");
			if (mat5) {
				mat5->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, (vars.visuals.MollyWarning & 32));
				mat5->SetMaterialVarFlag(MATERIAL_VAR_NO_DRAW, (vars.visuals.MollyWarning & 16));
			}

			if (mat6) {
				mat6->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, (vars.visuals.MollyWarning & 32));
				mat6->SetMaterialVarFlag(MATERIAL_VAR_NO_DRAW, (vars.visuals.MollyWarning & 16));
			}
