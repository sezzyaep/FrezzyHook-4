...
Vector Ragebot::HeadScan(animation* anims, int& hitbox, float& best_damage, float min_dmg) {
	...
	if (CurrentSettings().dynamicdamage) {
		if (min_dmg > health )
			min_dmg = health ;
	}
...
Vector Ragebot::FullScan(animation* anims, int& hitbox, float& simtime, float& best_damage, float min_dmg) {
	...
	if (best_damage > anims->player->GetHealth() / 2.f)
		target_lethal = true;
	RestorePlayer(anims);
	return best_point;
}

void FixDamage(int& Dmg) {
	if (csgo->weapon->IsAWP() && Dmg > 95) {
		Dmg += 8;
	}
}

ScanReturn Ragebot::GetAimVector(IBasePlayer* pTarget, float& simtime, Vector& origin, animation*& best_anims, int& hitbox)
{
	ScanReturn returned;
	returned.Aimpos = Vector(0, 0, 0);
	returned.PredictedDmg = 0;
	...
	if (!vars.ragebot.posadj) {
		if (record.has_value())
		{
			...
			returned.Aimpos = FullScan(record.value(), hitbox, simtime, damage, m_damage);;
			returned.PredictedDmg = damage;
			return returned;

		}
	}

	if ((vars.ragebot.backshoot_bt || vars.keybind.onshot->active) && !vars.ragebot.force_body->active) {
...
static const int total_seeds = 170;
...
HCreturn HitTraces(animation* _animation, const Vector position, const float chance, int box)
{
	HCreturn rn;
	build_seed_table();
	rn.Valid = false;
	...
	if (id == WEAPON_REVOLVER) {
		rn.Valid = weapon_inaccuracy < .0038f;
		rn.Hitchance = weapon->hitchance();
		return rn;
	...
	for (auto i = 0u; i < total_seeds; i++)  // NOLINT(modernize-loop-convert)
	{
		...
		end = start + end.Normalize() * weapon->GetCSWpnData()->m_flRange;
		...
		if (static_cast<float>(current) / static_cast<float>(total_seeds) >= chance) {
			rn.Valid = true;
			rn.Hitchance = int((static_cast<float>(current) / static_cast<float>(total_seeds)) * 100);
			return rn;
		}
	...
	rn.Valid = static_cast<float>(current) / static_cast<float>(total_seeds) >= chance;
	rn.Hitchance = int((static_cast<float>(current) / static_cast<float>(total_seeds)) * 100);
	return rn;
}

HCreturn Ragebot::Hitchance(Vector Aimpoint, bool backtrack, animation* best, int& hitbox)
{
	HCreturn rn;
	bool r8 = csgo->weapon->GetItemDefinitionIndex() == weapon_revolver;

	if (r8) {
		rn.Valid = csgo->weapon->hitchance() > CurrentSettings().hitchance * (1.7 * (1.f - r8));
		rn.Hitchance = csgo->weapon->hitchance();
		return rn;
	}
	else
		return HitTraces(best, Aimpoint, csgo->weapon->IsZeus() ? 0.4f : CurrentSettings().hitchance / 100.f, hitbox);
...
int Ragebot::GetTicksToStop() {
	...
	auto GetLowestSpeed = []() -> const float
	{
		const auto max_speed = csgo->weapon->GetZoomLevel() > 0 ? csgo->weapon->GetCSWpnData()->m_flMaxSpeedAlt : csgo->weapon->GetCSWpnData()->m_flMaxSpeed;
		return max_speed / 2.f;
	};
	Vector vel = csgo->vecUnpredictedVel;
	int ticks_to_stop = 0;
	auto Lowest = GetLowestSpeed();
	while (true)
	{
		if (vel.Length2D() <= Lowest)
			break;
...
void Ragebot::Run()
{
	...
	bool is_able_to_shoot = IsAbleToShoot();// || (csgo->weapon->GetItemDefinitionIndex() == WEAPON_REVOLVER && CockRevolver());
	...
	for (auto i = 1; i <= interfaces.global_vars->maxClients; i++)
	{
		...
		target_lethal = false;
		returnn = GetAimVector(pEntity, simtime, minus_origin, anims, box);
		Vector aim_position = returnn.Aimpos;
	...
	static bool OnPeekAA = false;
	did_dt = false;
	bool StopAgain = true;
	auto Cfg = CurrentSettings();
	if (hitchanced) {
		if (!vars.movable.peeking)
		{
			vars.movable.peeking = true;
		}
	}
	if (Cfg.quickstop && Cfg.quickstop_options & 8 && hitchanced && csgo->local->GetFlags() & FL_ONGROUND) {

		if (!csgo->weapon->IsZeus()) {
		
			if (Cfg.quickstop) {
				bool ShouldStop = GetTicksToShoot() <= GetTicksToStop();

				if (Cfg.quickstop_options & 1)
					ShouldStop = true;

				if (Cfg.quickstop_options & 2 && !is_able_to_shoot) {
					ShouldStop = false;
				}

				if (Cfg.quickstop_options & 4 && !target_lethal) {
					ShouldStop = false;
				}
	...
	if (hitbox != -1 && target_index != -1 && best_anims && current_aim_position != Vector(0, 0, 0))
	{
		...
		csgo->EnemyVisible = true;
	

		if (!OnPeekAA && vars.movable.OnPeekAA->active) {
			vars.antiaim.inverter->active = !vars.antiaim.inverter->active;
			OnPeekAA = true;
		}
		...
		auto can_shoot_if_fakeduck = csgo->CSI_FD;
		auto THC = Hitchance(current_aim_position, false, best_anims, hitbox);
		bool htchance = THC.Valid;
		
		/*if (vars.ragebot.active_index == 2 || 3 || 5)
		{
			if (!vars.ragebot.autoscope && csgo->weapon->GetZoomLevel() == 0)
			{
				htchance = 18.f;
			}
		}*/
		...
		if (StopAgain && csgo->local->GetFlags() & FL_ONGROUND) {
		...
		if (vars.ragebot.autoshoot) {
			...
			if (htchance && can_shoot_if_fakeduck && is_able_to_shoot)
			{
				...
				else if (vars.ragebot.double_tap->active)
				{
					csgo->send_packet = true;
				}
				if (vars.antiaim.onshottt)
		...
		if (htchance && can_shoot_if_fakeduck && is_able_to_shoot)
		{
			if (csgo->cmd->buttons & IN_ATTACK) {
				...
				tick_record record;
				snapshot.player = best_anims->player;
				snapshot.hitchance = clamp(THC.Hitchance, 0, 100);
				snapshot.predicteddmg = returnn.PredictedDmg < 0 ? 90 + rand() % 23 : returnn.PredictedDmg;
				...
				if (vars.ragebot.shotrecord) {
					...
					Chams->AddMatrixToHitChams(best_anims);
				}
	...
	if (is_able_to_shoot && csgo->cmd->buttons & IN_ATTACK)
	{
		...
		if (vars.movable.OnshotAA && !SwitcheedAA) {
			bool AAT = vars.antiaim.inverter->active;
			if (AAT) {
		...
		shot = true;
		csgo->visiblee = false;
		//static auto T = interfaces.cvars->FindVar("weapon_recoil_scale");
		//csgo->cmd->viewangles -= csgo->local->GetPunchAngle() * T->GetFloat();

	}
	else if (!(csgo->cmd->buttons & IN_ATTACK) && SwitcheedAA && is_able_to_shoot) {
		SwitcheedAA = false;
	}
