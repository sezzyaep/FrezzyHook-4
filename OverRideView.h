...
void Thirdperson()
{
	...
	if (csgo->local->isAlive() && enabledtp)
	{
		...
		csgo->CameraPos = origin - ((cam_forward * interfaces.input->m_vecCameraOffset.z) * trace.fraction);
		interfaces.input->m_vecCameraOffset.z *= trace.fraction;
	}
	else if (interfaces.input->m_fCameraInThirdPerson)
	{
		csgo->CameraPos = csgo->local->GetEyePosition();
		interfaces.input->m_fCameraInThirdPerson = false;
	}
...
void __stdcall Hooked_OverrideView(void* pSetup)
{
	...
	if (interfaces.engine->IsInGame() && interfaces.engine->IsConnected())
	{
		if (csgo->local && csgo->local->isAlive()) {
			...
			//if (vars.visuals.ScopeTHing > 0 && csgo->local->IsScoped())
			//	Setup->m_iEdgeBlur = 0;
