...
class c_childlist : public c_element {
private:
	std::string label;
...
public:
	c_childlist(string str, void* value, vector<string> elements, float height, bool(*should_render)() = nullptr) {
		label = str;
		this->type = listbox;
		this->height = height;
		this->value = value;
		...
		total_elements_size = elements.size() * 24.f;
	}
	...
	int get_total_offset() {
		if (should_render)
			if (!should_render())
				return 0;
		return height - 10 + (label.size() > 0) * after_text_offset;
	};
};