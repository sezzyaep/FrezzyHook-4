...
void LogHits(IGameEvent* Event)
{
	...
	if (entityid == interfaces.engine->GetLocalPlayer())
	{
		...
		if (nUserID || nDead)
		{
			...
			*Message += " health remaining | aimed at ";


			if (shot_snapshots.size() > 0)
			{
				auto snapshot = shot_snapshots.front();
				if (snapshot.player == interfaces.ent_list->GetClientEntity(interfaces.engine->GetPlayerForUserID(nDead)))
				{
					*Message += snapshot.hitbox_where_shot + " | bt: " + to_string(snapshot.backtrack) + " | hc: " + to_string(snapshot.hitchance) + " | dmg: " + to_string(snapshot.predicteddmg);

					Ragebot::Get().last_hitted[snapshot.player->GetIndex()] = snapshot;
					csgo->damagedshots++;
...
void player_hurt(IGameEvent* game_event) {
	...
	if (entityid == interfaces.engine->GetLocalPlayer())
	{
		if (vars.visuals.hitmarker_sound > 0)
		{
			switch (vars.visuals.hitmarker_sound)
			{
			case 1:
				interfaces.engine->ClientCmd_Unrestricted("play buttons/arena_switch_press_02.wav", 0);
				break;
			case 2:
				interfaces.engine->ClientCmd_Unrestricted("play flick.wav", 0);
				break;
			case 3:
				interfaces.engine->ClientCmd_Unrestricted("play cod.wav", 0);
				break;
			}
		}
