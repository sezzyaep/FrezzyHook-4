...
bool c_keybind::update()
{
	...
	if (binder.open) {
		...
		pos.y += size.y;
		h = g_mouse.x >= pos.x + e&& g_mouse.y >= pos.y
			&& g_mouse.x <= pos.x + e+ size.x && g_mouse.y < pos.y + size.y;
		if (wnd->is_click() && h) {
			binder.open = false;
			wnd->g_active_element = nullptr;
			wnd->reset_mouse();
		...
		h = g_mouse.x >= pos.x + e&& g_mouse.y >= pos.y
			&& g_mouse.x <= pos.x + size.x + e&& g_mouse.y < pos.y + size.y * (binder.elements.size() + 1);
		...
		for (size_t i = 0; i < binder.elements.size(); i++) {
			pos.y += 20;
			h = g_mouse.x >= pos.x + e && g_mouse.y >= pos.y
				&& g_mouse.x <= pos.x + size.x + e && g_mouse.y < pos.y + size.y;
			if (h) {
...
bool c_keybind::hovered()
{
	...
	auto size = Vector2D(g_size, 20);
	auto e = 270 - 20;
	return g_mouse.x > pos.x + e && g_mouse.y > pos.y
		&& g_mouse.x < pos.x + e + size.x && g_mouse.y < pos.y + size.y;
}
...
void c_keybind::render() {
	...
	auto clr = vars.movable.colormode == 2 ? color_t(35, 35, 35, alpha) : color_t(17, 10, 47, alpha);
	auto clr2 = vars.movable.colormode == 2 ? color_t(35, 35, 35, alpha) : color_t(17, 10, 47, alpha);


	g_Render->DrawString(pos.x - 7 - 270 + 20, pos.y - 2, color_t(255, 255, 255, alpha),
	...
	//	color_t(210, 145, 255, alpha));
	string format = label + ": %s";
	string B = KeyStrings[bind->key];
	if (B == "")
	{
		B = "-";
	}

	g_Render->DrawString(pos.x + size.x / 2 - 7, pos.y + size.y / 2 - 10, color_t(255,255,255, alpha),
		render::centered_x, fonts::Keybinds_,B.c_str());
	...
	if (binder.open) {
		...
		pos.y += size.y;
		auto size = Vector2D(g_size, 20);
		//g_Render->Rect(pos.x - 1, pos.y - 1, size.x + 2, (size.y * (binder.elements.size() + 1)) + 2, color_t(210, 145, 255, alpha));
