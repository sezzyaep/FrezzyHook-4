...
bool c_colorpicker::update()
{
	...
	if (dialogue.open) {
		...
		if (dragging_slider > 0) {
			...
			float newpos_y = g_mouse.y - (pos.y); //retarded coders kys if u coded this [i fixed thisi crap]
...
bool c_colorpicker::hovered()
{
	...
	pos.y += 2;
	auto size = Vector2D(20, 14);
	return g_mouse.x > pos.x + 60 - 4 - 10 && g_mouse.y > pos.y
		&& g_mouse.x < pos.x + 60 + size.x + 4 - 10 && g_mouse.y < pos.y + size.y + 4;
}
...
//	auto alpha = (int)(wnd->get_transparency() * 2.55f);
	color_faded.set_alpha(wnd->get_transparency() * 2.55f);

	g_Render->DrawString(pos.x + 1 + 60, pos.y + 2 + 5, color_faded, render::centered_x | render::centered_y, fonts::NewIconsNotify, "A");
	if (dialogue.open) {
		pos.y += size.y + 2;
		size = Vector2D(80, 20);
